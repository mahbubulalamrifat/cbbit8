...
class ScheduleEmailIaccessInternetRequest extends Controller
{
    ...
    public static function SEND(){
        ...
        $counter = ScheduleEmailIaccessInternetReq::whereNull('status')->count();

        if( !empty($counter) ){

            for($i=1; $i <= $counter; $i++){

                $item = ScheduleEmailIaccessInternetReq::whereNull('status')->first();
                ...
                Mail::send('mail.app-complain-action', $mailData, function ($message) use ($mailData) {
                    ...
                    if ( !empty($mailData['document']) ) {
                        ...
                        $message->attach( storage_path('images/iaccess/email/'.$mailData['d'].'pdf') );
                    }
                });
