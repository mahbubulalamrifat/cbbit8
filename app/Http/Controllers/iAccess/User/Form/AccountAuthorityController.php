...
class AccountAuthorityController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($lastData && $lastData->number){
            $number = CommonController::RunningNumberGenerate('AR', $lastData->number);
        }else{
            $number = CommonController::RunningNumberGenerate('AR');
        }
        ...
        $savedData = iaccessAccountRequest::with('manager')->find($data->id);
        ...
        $sub = $savedData->name.' : Account / Authority Request';
        ...
        $mailData = [
            ...
            'applicant_access_for'=> $savedData->access_for ?? '',
            'applicant_modules'=> $savedData->modules ?? '',
        ...
        //dd($mailData);
        //CommonController::MailSendAccountAuthorityRequest($mailData);
    ...
    // MailSendAccountAuthorityRequest
    public static function MailSendAccountAuthorityRequest($mailData){
       
        Mail::send('iaccess.user.email.acc-authority.approve', compact('mailData'), function ($message) use ($mailData) {
            $message->to( $mailData['to'] );
            $message->subject( $mailData['sub'] );
            $message->from( 'it-noreply@cpbangladesh.com' );
        });
