...
class EmailRequestController extends Controller
{
    ...
    public function index(){
        ...
        $allQuery =  iaccessEmailRequest::with('manager', 'buhead', 'verify', 'makby')->whereNotNull('verify_status');
    ...
    public function email_resend(){
        ...
        if($type == 'v'){
            // verify
            $data = iaccessEmailRequest::find($id);
            self::verifyEmail($data);
        }elseif($type == 'm'){
            // manager
            $data = iaccessEmailRequest::with('manager')->find($id);
            self::managerEmail($data);
        }elseif($type == 'b'){
            // B U
            $data = iaccessEmailRequest::with('buhead', 'manager')->find($id);
            self::buEmail($data);
        }
    ...
    public static function verifyEmail($data){
        ...
        $sub = $data->name.' : Email Request Update';
        ...
        Mail::send('iaccess.user.email.email-request.verify', compact('mailData'), function ($message) use ($mailData) {
    ...
    public function get_app_details($id){
        ...
        $data = iaccessEmailRequest::with('manager', 'buhead', 'verify')->where('id', $id)->first();

        // PDF Generate
        $pdf = PDF::loadView('iaccess.admin.pdf.emailRequestForm', compact('data'))
        ->setOption('footer-font-size', 6)
        ->setOption('margin-bottom', 4)
        ->setOption("encoding", "UTF-8");

        $filename = 'email_'.Str::random(10);
        $pdf->save(public_path('images/iaccess/pdf/email/'.$filename.'.pdf'));

        $path = 'images/iaccess/pdf/email/'.$filename.'.pdf';
