...
class GuestRequestController extends Controller
{
    ...
    public function index(){
        ...
        $allQuery =  iaccessGuestRequest::with('manager', 'buhead', 'verify', 'makby')->whereNull('verify_status');
    ...
    public function remarks_action(Request $request){
       ...
       //dd($request->all());
        ...
        // IT Approve
        $data = iaccessGuestRequest::with('manager', 'buhead')->where('id', $request->id)->first();
    ...
    public function email_resend(){
        ...
        if($type == 'v'){
            // verify
            $data = iaccessGuestRequest::find($id);
            self::verifyEmail($data);
        }elseif($type == 'm'){
            // manager
            $data = iaccessGuestRequest::with('manager')->find($id);
            self::managerEmail($data);
        }elseif($type == 'b'){
            // B U
            $data = iaccessGuestRequest::with('buhead', 'manager')->find($id);
            self::buEmail($data);
        }
    ...
    public static function verifyEmail($data){
        ...
        $sub = $data->name.' : Guest User Request Update';
        ...
        Mail::send('iaccess.user.email.guest-request.verify', compact('mailData'), function ($message) use ($mailData) {
    ...
    public function get_app_details($id){
        ...
        $data = iaccessGuestRequest::with('manager', 'buhead', 'verify')->where('id', $id)->first();

        // PDF Generate
        $pdf = PDF::loadView('iaccess.admin.pdf.guestRequestForm', compact('data'))
        ->setOption('footer-font-size', 6)
        ->setOption('margin-bottom', 4)
        ->setOption("encoding", "UTF-8");

        $filename = 'guest_request_'.Str::random(10);
        $pdf->save(public_path('images/iaccess/pdf/guest/'.$filename.'.pdf'));

        $path = 'images/iaccess/pdf/guest/'.$filename.'.pdf';
