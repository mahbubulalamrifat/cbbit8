  ...
  <div>
    <v-card>
      <v-card-title class="justify-center">
        <v-row>
          <v-col cols="10"> All Employees Data </v-col>
          <v-col cols="2"> </v-col>
        </v-row>
      </v-card-title>
          ...
          <v-form v-model="valid" ref="form">
            <form @submit.prevent="editmode ? updateData() : createData()">
              <v-row>
                <v-col cols="12">
                  <v-text-field
                    v-model="form.today_temp"
                    label="Enter Today Temparature"
                    :rules="allRules"
                    required
                    outlined
                    dense
...
export default {
  ...
  methods: {
    ...
    getCheckpointData() {
      axios
        .get(this.currentUrl + "/check_point")
        .then((response) => {
