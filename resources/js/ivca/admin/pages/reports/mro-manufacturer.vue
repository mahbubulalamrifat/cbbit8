          ...
          <table class="table table-bordered table-responsive">
            ...
            <tbody>
              <tr v-for="singleData in allData.data" :key="singleData.id">
                ...
                <td>
                  <span
                    v-for="(item, i) in singleData.audits_manufacturer"
                    :key="i"
                  >
                    <!-- {{ item.id }}, -->
                    <v-btn
                      v-if="item.status == 1"
                      @click="singleAuditShow(item.id)"
                      small
                      class="info m-1"
                      >Audit by
                      <span v-if="item.auditorData">
                        {{ item.auditordata.name }}
                      </span></v-btn
                    >
                  </span>
                </td>
                ...
                <td>
                  ...
                  <v-btn
                    v-if="singleData.audits_manufacturer.length > 0"
                    @click="auditReportShow(singleData.token)"
                    small
                    class="info"
                    >Summary</v-btn
                  >
    ...
    <v-dialog v-model="data_view_modal" max-width="1100px">
      <v-card>
        <v-card-title class="justify-center">
          <v-row>
            <v-col cols="10"> Single Audit Report </v-col>
            <v-col cols="2">
              <v-btn
                @click="data_view_modal = false"
                color="red lighten-1"
                small
                text
                class="float-right"
