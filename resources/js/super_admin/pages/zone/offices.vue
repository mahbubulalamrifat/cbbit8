      ...
      <v-card>
        <v-card-title class="justify-center">
          <v-row>
            ...
            <v-col cols="2">
              <v-btn
                @click="(dataModalDialog = false), getAllOffices()"
                color="red lighten-1"
                small
                text
                class="float-right"
...
export default {
  ...
  methods: {
    ...
    getAllOffices() {
      axios
        .get(this.currentUrl + "/alloffices")
        .then((response) => {
    ...
    editDataModel(singleData) {
      ...
      axios
        .get(this.currentUrl + "/edit/" + singleData.id)
        .then((response) => {
    ...
    updateData() {
      ...
      // request send and get response
      this.form
        .put(this.currentUrl + "/update/" + this.form.id)
        .then((response) => {
          // Loading Animation
          this.dataModalLoading = false;
          // Input field make empty
