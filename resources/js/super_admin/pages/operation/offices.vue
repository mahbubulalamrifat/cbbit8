    ...
    <div>
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-8">
                        <h3 class="card-title">Operation Offices List</h3>
                        <span class="text-danger small" v-if="allOffices.length">
                            Remaining Operation Offices : <b>{{ allOffices.length }}</b>
                        </span>
                    </div>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <tr v-for="singleData in allData.data" :key="singleData.id">
                                    <td>{{ singleData.id }}</td>
                                    <td><span v-if="singleData.operation">{{ singleData.operation.name }}</span><span
                                            v-else class="text-danger">N/A</span></td>
                ...
                <v-card-text>
                    <v-form v-model="valid">
                        <form @submit.prevent="editmode ? updateData() : createData()">
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <div class="text-danger" v-if="form.errors.has('operation_id')"
                                        v-html="form.errors.get('operation_id')" />
                                    <v-select label="Select Operation" outlined v-model="form.operation_id"
                                        :items="allOperations" item-text="name" item-value="id">
                                    </v-select>
                                </v-col>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                //current page url
                currentUrl: '/super_admin/operation/offices',
        ...
        methods: {
            ...
            // getAllZons
            getAllZons() {
                axios.get(this.currentUrl + '/alloperations').then(response => {
            ...
            editDataModel(singleData) {
                ...
                axios.get(this.currentUrl + '/edit/' + singleData.id).then(response => {
                    ...
                    this.form.operation_id = singleData.operation_id
