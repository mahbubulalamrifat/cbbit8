<template>
  <div>
      ...
      <v-card-text v-if="complainDeta" class="table-responsive">
        ...
        <table class="table mb-0">
          <tr>
            ...
            <td>
              <v-btn
                v-if="complainDeta.category"
                @click="modifyDialogShow()"
                color="info"
                small
                elevation="20"
              >
              ...
              <v-btn @click="modifyDialogShow()" v-else class="error"
                >N/A</v-btn
              >
            </td>
            <th>Subcategory:</th>
              ...
              <span v-if="complainDeta.document">
                ...
                >
                  <v-btn
                    ...
                    @click="imageGetFile(docPath + complainDeta.document)"
                    small
                    class="ma-1"
                    color="primary"
                  >
        ...
        <div v-if="complainDeta.remarks" class="mb-2 rem_bg_color">
          <div v-for="(item, index) in complainDeta.remarks" :key="index">
            <!--Start remarks -->
            <table
              class="table mb-1 text-white rounded border-bottom border-danger"
            >
              <!-- remarks -->
              <tr>
                ...
                <td>
                  <span v-if="item.document">
                    <span
                      v-if="item.document && item.document.includes('.pdf')"
                    >
                      <v-btn
                        v-if="item.document"
                        :loading="pdfReadyLoading"
                    ...
                    >
                      <v-btn
                        v-if="item.document"
                        :loading="imageReadyLoading"
                        @click="imageGetFile(docPath + item.document)"
                        small
                        class="ma-1"
                        color="primary"
                      >
              ...
              <tr
                v-if="
                  (item.process == 'Damaged' ||
                    item.process == 'Partial Damaged') &&
                  complainDeta.damage &&
                  complainDeta.damage.apply_by
                "
                class="bg-info"
              >
                ...
                <td>
                  <span
                    v-if="complainDeta.damage.apply_at"
                    class="text-warning"
                    >{{
                      complainDeta.damage.apply_at
              ...
              <tr v-if="item.mail">
                <th>E-Mail:</th>
                <td>
                  <span v-if="item.mail.status" class="text-success"
            ...
            <div v-if="item.process == 'HO Service'">
              <div
                v-for="(item, index) in complainDeta.ho_remarks"
                :key="index"
              >
        ...
        >
          ...
          <tr>
            ...
            <td colspan="3">
              <button
                class="btn btn-secondary btn-sm"
                v-if="complainDeta.damage.makby"
                @click="currentUserView(complainDeta.damage.makby)"
              >
                <v-avatar size="20">
                  <img
                    v-if="complainDeta.damage.makby.image"
                    :src="
                      '/images/users/small/' + complainDeta.damage.makby.image
                    "
                    alt="image"
                  />
                </v-avatar>
                {{ complainDeta.damage.makby.name }}
              </button>
            </td>
            <th>Action At:</th>
            <td colspan="3">
              <span v-if="complainDeta.damage.created_at">{{
                complainDeta.damage.created_at | moment("MMMM Do YYYY, h:mm a")
              }}</span>
            </td>
          </tr>
          <tr class="bg-info">
            ...
            <td>
              <span
                v-if="complainDeta.damage.updated_at"
                class="text-warning"
                >{{
                  complainDeta.damage.updated_at
                    | moment("MMMM Do YYYY, h:mm a")
                }}</span
              >
            </td>
        ...
        >
          ...
          <tr>
            ...
            <td colspan="3">
              <button
                class="btn btn-secondary btn-sm"
                v-if="complainDeta.delivery.makby"
                @click="currentUserView(complainDeta.delivery.makby)"
              >
                <v-avatar size="20">
                  <img
                    v-if="complainDeta.delivery.makby.image"
                    :src="
                      '/images/users/small/' + complainDeta.delivery.makby.image
                    "
                    alt="image"
                  />
                </v-avatar>
            ...
            <th>Action At:</th>
            <td colspan="3">
              <span v-if="complainDeta.delivery.created_at">{{
                complainDeta.delivery.created_at
                  | moment("MMMM Do YYYY, h:mm a")
              }}</span>
            </td>
          </tr>
          ...
          <tr v-if="complainDeta.delivery.mail">
            <th>E-Mail:</th>
            <td colspan="3">
              <span v-if="complainDeta.delivery.mail.status"
                >Successfully Sent</span
              >
              <span v-else class="text-warning">Sending</span>
              <v-btn
                @click="mailSendManual(complainDeta.delivery.mail.id)"
                small
                class="float-right"
                elevation="20"
              >
            ...
            <td colspan="3">
              <span v-if="complainDeta.delivery.mail.status">{{
                complainDeta.delivery.mail.updated_at
                  | moment("MMMM Do YYYY, h:mm a")
              }}</span>
              <span v-else class="text-warning">Sending</span>
            </td>
          ...
          <tr>
            ...
            <td>
              <span
                v-if="complainDeta.delivery.updated_at"
                class="text-warning"
                >{{
    ...
    <!-- Modify Dialog -->
    <cat-sub-modify-dialog
      v-if="CurrentComDataModify"
      :comData="CurrentComDataModify"
      :key="comModifyDialogKey"
