                ...
                <v-card-text>
                    ...
                    <table class="table mb-0">
                        <tr>
                            <th>Complain No:</th>
                            <td>
                                <div class="pa-1 success rounded-pill h6 text-white text-center">{{ comData.id }}</div>
                            </td>
                            <th>Category:</th>
                        ...
                        <form @submit.prevent="actionStore()">
                            ...
                            <v-row align-content="center">
                                ...
                                <v-card-text v-if="remainingStock">
                                    <p>Remaining Stock:</p>
                                    <v-btn-toggle multiple v-for="(item, index) in remainingStock" :key="index">
                                        <v-btn v-if="item[0] && item[0].category" @click="selectStockProduct(item)"
                                            class="ma-2" outlined small>{{ item[0].category.name }} :
                                            <b>{{ item.length }}</b>
                                        </v-btn>
                                    </v-btn-toggle>
                                ...
                                <v-card-text v-else>
                                    <div class="p-5 my-5">
                                        <p class="text-center h1">Loading.. <v-icon color="success" size="100">mdi
                                ...
                                <!-- <v-col cols="12" lg="4">
                                    ...
                                    <v-text-field v-model="form.rec_name"
                                        :rules="[v => !!v || 'Reciver Name is required!']" label="Receiver name"
                                        required></v-text-field>
                                </v-col>
                                <v-col cols="12" lg="4">
                                    <div class="small text-danger" v-if="form.errors.has('rec_contact')"
                                        v-html="form.errors.get('rec_contact')" />
                                    <v-text-field type="number" v-model="form.rec_contact" :rules="numberRules"
                                        label="Receiver Contact" required></v-text-field>
                                </v-col>
                                <v-col cols="12" lg="4">
    ...
    export default {
        ...
        methods: {
            ...
            selectStockProduct(val) {
                ...
                //this.secectedProID = selectItemid
                ...
                for (let i = 0; i < this.initialSelectProduct.length; i++) {
                    let optionData = {
                        value: this.initialSelectProduct[i].id,
                        text: this.initialSelectProduct[i].id + '. ' + this.initialSelectProduct[i].category.name +
                            ' -- ' + this.initialSelectProduct[i]
                            .name + ' -- SL: ' + this.initialSelectProduct[i].serial + ' -- P.O: ' + this
                            .initialSelectProduct[i].po_number
                    }
