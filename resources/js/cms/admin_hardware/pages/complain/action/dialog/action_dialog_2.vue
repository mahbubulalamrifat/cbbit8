    ...
    <v-dialog v-model="actionDailog" scrollable>
      <v-card>
        <v-card-title class="justify-center">
          <v-row>
            <v-col cols="10"> Complain Actions (2) : </v-col>
            <v-col cols="2">
              <v-btn
                @click="actionDailog = false"
        ...
        <v-card-text>
          <table class="table mb-0">
            <tr>
              ...
              <td>
                <div
                  class="pa-1 success rounded-pill h6 text-white text-center"
                >
                  {{ comData.id }}
                </div>
              </td>
              <th>Category:</th>
              <td>
                <span v-if="comData.category">{{ comData.category.name }}</span>
              </td>
              <th>Subcategory:</th>
              <td>
                <span v-if="comData.subcategory">{{
                  comData.subcategory.name
                }}</span>
              </td>
            </tr>
          </table>
              ...
              <v-row align-content="center">
                ...
                <v-col cols="12" lg="6" v-if="applicableOptions.length">
                  <v-autocomplete
                    ...
                    dense
                    v-model="form.damaged_reason"
                    label="Damaged Reason"
                    :rules="[(v) => !!v || 'Damaged Reason is required!']"
                    outlined
                    required
                  ></v-autocomplete>
                </v-col>
                <v-col cols="12" lg="6" v-if="applicableOptions.length">
                  ...
                  <v-radio-group v-model="form.applicable_type" row>
                    <v-radio
                      v-for="(item, k) in applicableOptions"
                      :key="k"
                      :label="`${item.label}`"
                      :value="`${item.value}`"
                      :color="`${item.color}`"
  ...
  methods: {
    setProcessOptions() {
      ...
      if (crpro == "Processing" && this.dhkZoneAccess) {
        ...
        (crpro == "Send Service" ||
          crpro == "Back Service" ||
          crpro == "Again Send Service" ||
          crpro == "Service Quotation") &&
        this.dhkZoneAccess
      ) {
        this.stepOptions = [
          {
    ...
    actionStore() {
      ...
      this.form
        .post(this.currentUrl + "/action_remarks")
        .then((response) => {
    ...
    getUserZone() {
      axios
        .get(this.currentUrl + "/get_user_zone_name")
        .then((response) => {
