                                ...
                                <tr v-for="singleData in allData.data" :key="singleData.id">
                                    <td>
                                        <img v-if="singleData.image" :src="imagePathSm + singleData.image" alt="image"
                                            class="m-1" height="100" width="100">
                                    ...
                                    <td>
                                        ...
                                        <div v-if="singleData.temporary==1">
                                            ...
                                            <v-btn depressed small color="indigo white--text" class="d-flex flex-end"
                                        ...
                                        <div v-if="singleData.temporary==0">
                                            ...
                                            <v-btn depressed small color="cyan white--text" class="d-flex flex-end"
                                                @click="temporaryChange(singleData)">
                                        ...
                                        <div v-if="singleData.last_use">
                                            ...
                                            <v-btn depressed small color="blue-grey white--text" class="d-flex flex-end"
                                    ...
                                    <td class="text-center">
                                        ...
                                        <v-btn v-if="isAdministrator()" @click="deleteDataTemp(singleData.id)" color="error" depressed small
                                            class="m-1">
                                            <v-icon left>mdi-delete-empty</v-icon> Delete
                                        </v-btn>

                                        <v-btn depressed small color="grey white--text"
                                            @click="deadlineModal(singleData)" class="m-1">
                ...
                <v-card-text>
                    <v-form v-model="valid" ref="form">
                        <form @submit.prevent="editmode ? updateData() : createData()">
                            ...
                            <v-row>
                                <v-col md="6" cols="12">
                                    <div class="text-danger" v-if="form.errors.has('number')"
                                        v-html="form.errors.get('number')" />
                                    <v-text-field v-model="form.number" label="Enter Car Number" :rules="carRules"
                                ...
                                <v-col md="6" cols="12">
                                    ...
                                    <v-text-field v-model="form.name" label="Enter Car Name" :rules="carRules" required>
                                    </v-text-field>
                                </v-col>
                            </v-row>
                            ...
                            <v-row>
                                <v-col cols="12" lg="6">
                                    <div class="text-danger" v-if="form.errors.has('remarks')"
                                        v-html="form.errors.get('remarks')" />
                                    <v-textarea outlined rows="2" v-model="form.remarks" label="Enter Car Details">
                                    </v-textarea>
                                </v-col>
                                ...
                                <v-col cols="12" lg="6">
                                    <div class="text-danger" v-if="form.errors.has('capacity')"
                                        v-html="form.errors.get('capacity')" />
                                    <v-text-field type="number" v-model="form.capacity" label="Enter Car capacity"
                                        :rules="carRules" required></v-text-field>
                                </v-col>



                                <v-col lg="6" cols="12">
                                    <div class="text-danger" v-if="form.errors.has('status')"
                                        v-html="form.errors.get('status')" />
                                    <v-select label="Car Status (Default Active)" v-model="form.status"
                                        :items="statusOptions" :rules="carRulesOption" required>
                                    </v-select>
                                </v-col>
                                <v-col lg="6" cols="12">
                                    <div class="text-danger" v-if="form.errors.has('temporary')"
                                        v-html="form.errors.get('temporary')" />
                                    <v-select label="Car Type" v-model="form.temporary" :items="typeOptions"
                                        :rules="carRulesOption" required>
                                    </v-select>
                                </v-col>
                            ...
                            <v-row>
                                ...
                                <v-col md="4">
                                    <v-file-input prepend-icon="mdi-camera"
                                        @change="uploadImageByName($event, 'image2')" label="Choose 2nd Image"
                                        accept=".jpg, .png, .jpeg">
                                    </v-file-input>
                                </v-col>
                                <!-- Image 2 -->
                                <v-col md="4">
                                    <v-file-input prepend-icon="mdi-camera"
                                        @change="uploadImageByName($event, 'image3')" label="Choose 3rd Image"
                                        accept=".jpg, .png, .jpeg">
                                    </v-file-input>
                                </v-col>
                            </v-row>
                    ...
                    <v-form v-model="valid" ref="form">
                        <form @submit.prevent="createDeadlineData()">
                            <v-row>
                                ...
                                <v-col cols="12">
                                    <v-menu v-model="menu" min-width="auto">
                                        <template v-slot:activator="{ on, attrs }">
                                            <v-text-field v-model="form.last_use" label="Enter Last Use Date"
                                ...
                                <v-btn type="submit" block depressed :loading="addCarpoolLoader" color="primary">
