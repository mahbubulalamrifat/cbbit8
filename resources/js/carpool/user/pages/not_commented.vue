<template>
    <div>
        <div v-if="allData">
            <div v-if="commentData.length > 0">
                <v-card v-for="singleData in commentData" :key="singleData.id" class="my-5">
                    <v-card-text>
                        <v-row>
                            <v-col md="5">
                                <v-img v-if="singleData.car" :src="imagePath + singleData.car.image" max-height="250"
                                    class="rounded-lg" @click="carDetails(singleData.car)" style="cursor:pointer">
                                </v-img>
                            </v-col>
                            ...
                            <v-col md="7">
                                <div class="d-flex justify-content-between align-center flex-wrap">
                                    ...
                                    <div>
                                        <v-badge
                                            :content="`Booked For `+ durationCal(singleData.start, singleData.end)+ ` Hours`"
                                            inline></v-badge>
                                    </div>
                                </div>
            ...
            <div v-else>
                <div class="p-5 my-5">
                    <p class="text-center text-info h1">All bookings were commented. Thank you. </p>
                </div>
            </div>
        </div>
        <div v-else>
                ...
                <v-card-text>
                    <v-form v-model="valid" ref="form">
                        <form>
                            ...
                            <v-row class="mb-5">
                                <v-col cols="12" md="6">
                                    <v-text-field label="Start Mileage" type="number" v-model="form.start_mileage"
                                        @input="validate(0)" :rules="firstValidation"></v-text-field>
                                </v-col>
                                <v-col cols="12" md="6">
                                    <v-text-field label="End Mileage" type="number" v-model="form.end_mileage"
                                        @input="validate(1)" :rules="secondValidation"></v-text-field>
                                </v-col>
                                <v-col cols="12" md="6">
                                    <v-text-field label="CNG Bill" type="number" v-model="form.gas"></v-text-field>
                                </v-col>
                                <v-col cols="12" md="6">
                                    <v-text-field label="Octane Bill" type="number" v-model="form.octane">
                                    </v-text-field>
                                </v-col>
                                <v-col cols="12">
                                ...
                                <v-col cols="12" class="text-center">
                                    ...
                                    <v-rating v-model="form.driver_rating" background-color="teal lighten-3"
                                        color="teal" large></v-rating>
                                </v-col>
                            </v-row>
        ...
        methods: {
            ...
            updateData(val) {
                ...
                if (val == 2) {
                    ...
                    if (this.form.start_mileage == null || this.form.end_mileage == null || this.form.gas == null ||
                        this.form.octane == null || this.form.toll == null) {
                        Swal.fire({
                            icon: 'error',
