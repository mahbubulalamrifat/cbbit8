  ...
  <div>
    <div v-if="allData">
      <div v-if="commentData.length > 0">
        <v-card
          v-for="singleData in commentData"
          :key="singleData.id"
          class="my-5"
        >
          <v-card-text>
            <v-row>
              <v-col md="5" style="position: relative">
                <v-img
                  v-if="singleData.driver"
                  :src="imagePathDriver + singleData.driver.image"
                  max-width="100"
                  style="
                    border-radius: 50%;
                    position: absolute;
                ...
                <v-img
                  v-if="singleData.car"
                  :src="imagePath + singleData.car.image"
                  max-height="250"
                  class="rounded-lg"
                  @click="carDetails(singleData.car)"
              ...
              <v-col md="7">
                ...
                >
                  ...
                  <div>
                    <v-badge
                      :content="
                        `Booked For ` +
                        durationCal(singleData.start, singleData.end) +
                        ` Hours`
                      "
                      inline
                    ></v-badge>
                ...
                <v-col cols="12"
                  >Destination : {{ singleData.destination }}</v-col
                >
                <v-col cols="12">Purpose : {{ singleData.purpose }}</v-col>
                <v-col cols="12"
                  >{{ singleData.start }} -- To -- {{ singleData.end }}
                </v-col>

                <v-btn
                  block
                  color="teal white--text"
                  @click="comment(singleData.id)"
                  >Comment</v-btn
                >
              </v-col>
            </v-row>
      ...
      <div v-else>
        <div class="p-5 my-5">
          <p class="text-center text-info h1">
            All bookings were commented. Thank you.
          </p>
        </div>
      </div>
    </div>
    ...
    <v-dialog v-model="dataModalDialog" max-width="700px">
      <v-card>
        <v-card-title justify-center>
          <v-row>
            <v-col cols="10"> Put Car Using Data </v-col>
            <v-col cols="2">
              <v-btn
        ...
        <v-card-text>
          <v-form v-model="valid" ref="form">
            <form>
              ...
              <v-btn class="float-right" color="error" @click="updateData(2)"
                >Close Permanently</v-btn
              >
            </form>
          </v-form>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    comment(id) {
      ...
      axios
        .get(this.currentUrl + "/prev_comment/" + id)
        .then((response) => {
    ...
    validate(index) {
      if (this.form.end_mileage != null) {
        ...
        else this.firstValidation = ["Must be less than End Mileage"];
      }
    },
    ...
    updateData(val) {
      ...
      if (val == 2) {
        if (
          this.form.start_mileage == null ||
          this.form.end_mileage == null ||
          this.form.gas == null ||
          this.form.octane == null ||
          this.form.toll == null
        ) {
          Swal.fire({
            icon: "error",
    ...
    durationCal(time1, time2) {
      ...
      let duration = this.$moment
        .utc(
          this.$moment(now, "YYYY/MM/DD HH:mm:ss").diff(
            this.$moment(then, "YYYY/MM/DD HH:mm:ss")
          )
        )
        .format("HH");
